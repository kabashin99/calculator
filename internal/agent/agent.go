...
func (a *Agent) worker(results map[string]float64) {
	for {
		...
		if err != nil {
			// log.Printf("Failed to fetch task: %v", err)  ПОТОМ ВКЛЮЧИТЬ
		...
		for _, depID := range task.DependsOn {
			for {
				if result, ok := results[depID]; ok {
					if task.Operand1 == 0 {
						task.Operand1 = result
					} else {
						task.Operand2 = result
					}
					log.Printf("Updated task %s with dependency %s: Arg1=%f, Arg2=%f",
						task.ID, depID, task.Operand1, task.Operand2)
					break
				}
...
func (a *Agent) executeTask(task *models.Task) float64 {
	log.Printf("Executing task: %s %f %s %f", task.Operation, task.Operand1, task.Operation, task.Operand2)
	time.Sleep(time.Duration(task.OperationTime) * time.Millisecond)

	switch task.Operation {
	case "+":
		return task.Operand1 + task.Operand2
	case "-":
		return task.Operand1 - task.Operand2
	case "*":
		return task.Operand1 * task.Operand2
